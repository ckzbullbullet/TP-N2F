        ...
        if isinstance(statement, six.string_types):
            ...
            def call_unit():
                try:
                    ...
                    #print("TypeError %s calling %s, arguments: %s. Program: %s" % (str(e), func, values, statement))
                    raise
            ret = call_unit
            ret._original_lisp_code = statement
            return ret
            ...
            for arg, sub in zip(x.args, short_tree[1:]):
                if callable(arg):
                    try:
                        ...
                        #print("Error while executing: %s, arg: %s, root_type: %s, args: %s" % (short_tree, arg, root_type, args))
                        raise
                else:
                    tp = arg
